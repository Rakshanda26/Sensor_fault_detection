...
class DataValidation:
    ...
    def is_required_columns_exists(self,base_df:pd.DataFrame,curr_df:pd.DataFrame,report_key_name:str)->bool:
        try:
    ...
    def initiate_data_validation(self)->artifact_entity.DataValidationAritfact:
        try:
            ...
            test_df_column_status= self.is_required_columns_exists(base_df=base_df, curr_df=test_df,report_key_name="missing_columns_within_test_dataset")

            if train_df_column_status:
                logging.info(f"As all column are available train df hence detecting data drift")
                self.data_drift(base_df=base_df, curr_df=train_df,report_key_name="data_drift_within_train_dataset")

            if test_df_column_status:
                logging.info(f"As all column are available test df hence detecting data drift")
            ...
            data_validation_artifact =artifact_entity.DataValidationAritfact(report_file_path=self.data_validation_config.report_file_path)
